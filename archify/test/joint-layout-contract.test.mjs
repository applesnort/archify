...
import { ChromeVisualBrowser, runBrowserCheck } from '../bin/visual-check.mjs';
...
test('normal browser evidence retains all six inspections and receipt mappings with the joint capability', async () => {
  ...
  try {
    ...
    const f = inspection();
    const result = await runBrowserCheck({ artifactPath, chromePath: '/fake/chrome', browserFactory: () => f.browser });
