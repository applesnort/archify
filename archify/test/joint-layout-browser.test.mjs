...
import { ChromeVisualBrowser, findChrome, runBrowserCheck } from '../bin/visual-check.mjs';
...
test('one joint wait preserves real Reader/Chrome convergence at the CLI boundary', {
  ...
  try {
    ...
    await t.test('CLI visits all six fresh states and captures immediate geometry before collecting metrics', async () => {
      ...
      ` });
      const states = [];
      const result = await runBrowserCheck({ artifactPath: instrumented, chromePath: chrome,
      ...
      assert.deepEqual(result.receipt.themeStates.viewports.map(value => [value.width, value.height, value.requestedTheme]),
        [[1440, 900, 'light'], [1440, 900, 'dark'], [2048, 1320, 'light'], [2048, 1320, 'dark']]);
    });
