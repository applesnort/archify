...
def main():
    ...
    identity={'candidate_sha256':digest(candidate),'target_sha':subprocess.check_output(['git','-C',str(a.repo_root),'rev-parse','HEAD'],text=True).strip(),'workflow':'validate, deliver, strict check, visual-check; identical supported native commands on A/B/C','warmup':'one recorded warmup per variant, then three balanced repetitions','timing':'parent monotonic command and end-to-end fixed pipeline wall; no model; screenshots included equally','quality':'CLI result only; semantic and actual visual review remain separate'}
    ...
    for repeat, order in [(0,'ABC'),(1,'ABC'),(2,'BCA'),(3,'CAB')]:
        for v in order:
            ...
            commands=[('validate',['validate','architecture',candidate,'--repo-root',a.repo_root,'--quality','showcase','--json']),('deliver',['deliver','architecture',candidate,artifact,'--repo-root',a.repo_root,'--quality','showcase','--json']),('check',['check',artifact,'--require-provenance']),('visual-check',['visual-check',artifact,'--require-provenance','--out-dir',out/'visual','--json'])]
